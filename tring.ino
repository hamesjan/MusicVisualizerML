...
void relu(float* input, int length) {
...
void forward_pass(float input[INPUT_SIZE], float output[OUTPUT_SIZE]) {
    ...
    for (int i = 0; i < HIDDEN_SIZE; i++) {
        ...
        hidden[i] += fc1_bias[i];
        hidden[i] = relu(hidden[i], HIDDEN_SIZE);
    }
    ...
    for (int i = 0; i < OUTPUT_SIZE; i++) {
        ...
        for (int j = 0; j < HIDDEN_SIZE; j++) {
            output[i] += fc2_weights[i][j] * hidden[j];
        }
        output[i] += fc2_bias[i];
...
// Map frequency to hue (0-255)
    ...
    // Normalize input if required, this example assumes input is already normalized
    ...
    USBSerial.println();
    color = matrix.colorHSV(r, g, b); // Convert hue to RGB color
